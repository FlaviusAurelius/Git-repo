...
Proof. simpl. reflexivity. Qed.
Example test_andb33: (andb3 false false true) = false.
Proof. simpl. reflexivity. Qed.
Example test_andb34: (andb3 false false false) = false.
Proof. simpl. reflexivity. Qed.
...
Definition ltb (n m : nat) : bool :=
  andb (n <=? m) (negb (n =? m)).
  (* negb (leb m n).
(*  if n is less than or equal to m and n is not m, then n is less than m *) *)
...
Theorem zero_nbeq_plus_1 : forall n : nat,
  0 =? (n + 1) = false.
Proof.
(* line 1, 2 does the same thing as 3, just a shorthand for writing line 3 *)
(* Downside is we lose the equation recording assumption 
we are making in each sub goal, which we get from eqn:E in line 3 *)
(*   intros [].  *)   (* line 1 *)
(*   intros [| n]. *)  (* line 2 *)
  intros n. destruct n as [| n'] eqn:E. (* line 3 *)
  simpl.
  reflexivity.
  simpl.
  reflexivity.
